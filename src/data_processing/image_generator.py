...
def merge_images(n,all_ids,src_dir,dest_dir,annotations,final_images,final_annotations,t_id,v,lock,allowed_overlap=2500):
    ...
    retain_image_ratio = 0.3 ## 40% chance you retain
    first = True
    ...
    ## del elements
    for index in sorted(list(to_remove),reverse=True):
        del all_ids[index]
...
def generate_inital_positions(avg_image_height=250,avg_image_width=150):
    start_x = np.random.RandomState().randint(0,100,1)[0]
    start_y = np.random.RandomState().randint(0, 100, 1)[0]
    all_positions = []
    print('{}:{}'.format(start_x,start_y))
    print('IMGE={}'.format(IMG_SHAPE))
...
def create_merged_images(n_generate,annotation,src_dir,dest_dir,t_id,v,lock,lowest_cat_n=3,high_cat_n=10):
    ...
    for i in range(n_generate):
        if not len(all_ids):
            all_ids = list(annotation.imgs.keys())
...
if __name__ == '__main__':
    ...
    dest_dir = '/home/shibintv_me/created_resize'
    if not os.path.exists(dest_dir):
        print('create dest dir')
        import sys
        sys.exit(-1)
    annotaion_file = '/home/shibintv_me/instances_train2019.json'
    img_dest_dir = '/home/shibintv_me/created_resize_mul'
    annotation = annotation = load_annotations(annotaion_file)
    #do_preprocessing_all_image(annotaion_file,src_dir,dest_dir)
    n_image_to_create = 3000
    all_process = []
    v = Value('i',0)
    lock = Lock()
    for i in range(10):
